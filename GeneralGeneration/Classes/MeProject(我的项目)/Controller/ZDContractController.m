...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"签约门店";
    //创建控件
...
-(void)createController{
    ...
    label.text = @"审核通过后，在【已签约门店】里显示，否则，不显示";
    _label = label;
...
-(void)submit{
    ...
    if ([distributionCompanyName isEqual:@"请选择门店"]) {
        [SVProgressHUD showInfoWithStatus:@"门店不能为空"];
        return;
    }
    //项目Id
    ...
    if([projectName isEqual:@""]){
        [SVProgressHUD showInfoWithStatus:@"项目名不能为空"];
        return;
    }
    //合同开始时间
