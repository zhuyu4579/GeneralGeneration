...
#import <Masonry.h>
#import "GKCover.h"
@interface ZDSelectStoreController ()<UISearchBarDelegate,UITableViewDelegate,UITableViewDataSource>{
    //页数
    NSInteger current;
}
//区域label
...
//门店列表
@property(nonatomic,weak)ZDStoreTableView *store;
//数据数组
...
@implementation ZDSelectStoreController
...
- (void)viewDidLoad {
    ...
    if (_storeBlocks) {
        _store.storeBlock = ^(NSDictionary *storeDicty) {
            _storeBlocks(storeDicty);
        };
    }
}
//下拉刷新
...
//上拉刷新
-(void)loadMoreData{
    [self.store.mj_footer beginRefreshing];
    [self loadData];
}
//列表的数据请求
-(void)loadData{
        ...
        [mgr POST:url parameters:paraments progress:nil success:^(NSURLSessionDataTask * _Nonnull task, NSDictionary *  _Nullable responseObject) {
            ...
            if ([code isEqual:@"200"]) {
                ...
                if (rows.count == 0) {
                    [self.store.mj_footer endRefreshingWithNoMoreData];
                }else{
                ...
                _store.storeArray =  [ZDStoreItem mj_objectArrayWithKeyValuesArray:_storeListArray];
                [_store reloadData];
...
-(void)createView{
    ...
    ZDStoreTableView *store = [[ZDStoreTableView alloc] initWithFrame:proView.bounds];
    _store = store;
    [proView addSubview:store];
