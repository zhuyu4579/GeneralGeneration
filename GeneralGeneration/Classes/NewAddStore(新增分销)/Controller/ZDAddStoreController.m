...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"新增分销";
...
-(UIView *)createSuccess{
    ...
    [buttonTwo setTitle:@"签约楼盘" forState:UIControlStateNormal];
...
-(void)success{
    ...
    if([address isEqual:@"请选择"]||!address){
        [SVProgressHUD showInfoWithStatus:@"分销位置不能为空"];
        return;
    }
    //分销地址
    NSString *addr = _addr.text;
    if([addr isEqual:@""]){
        [SVProgressHUD showInfoWithStatus:@"分销地址不能为空"];
        return;
    }
    //分销人数
    NSString *totalPeople = _totalPeople.text;
    if([totalPeople isEqual:@""]){
        [SVProgressHUD showInfoWithStatus:@"分销人数不能为空"];
        return;
    }
    //负责人
